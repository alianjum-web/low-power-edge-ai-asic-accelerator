...
module silicon_npu_tb #(
    ...
    logic                    act_wr_en;
    logic signed [WIDTH*2+$clog2(ARRAY_SIZE)+$clog2(DEPTH)-1:0] result;
    logic                    done;
    logic                    busy;
    ...
    initial begin
        ...
        // ---- Test 3: Most-negative values (signed boundary) ----
        // Audited 2026-09-07: signed boundary case, not unsigned all-ones
        // -- see rtl/silicon_npu.sv header comment.
        $display("Test 3: Most-negative values (signed boundary)");
        begin
            logic [WIDTH-1:0] min_bits;
            longint min_signed, expected;
            min_bits = {1'b1, {(WIDTH-1){1'b0}}};
            min_signed = -(longint'(1) << (WIDTH-1));
            for (int r = 0; r < DEPTH; r++)
                write_row(r, min_bits, min_bits);
            expected = 0;
            for (int r = 0; r < DEPTH; r++)
                for (int c = 0; c < ARRAY_SIZE; c++)
                    expected += min_signed * min_signed;
        ...
        // ---- Test 5: Signed cross-sign product ----
        // Direct evidence of the audited signed fix: negative * positive
        // must sum negative.
        $display("Test 5: Signed cross-sign product (-5 * 3 per element)");
        begin
            logic [WIDTH-1:0] neg5, three;
            neg5  = WIDTH'(0) - WIDTH'(5);
            three = WIDTH'(3);
            for (int r = 0; r < DEPTH; r++)
                write_row(r, neg5, three);
            run_and_check(DEPTH * ARRAY_SIZE * (-15));
        end
