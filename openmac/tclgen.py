...
def gen_openlane_config(cfg: FlowConfig, verilog_path: str = "") -> str:
    ...
    Default verilog_path resolves to this repo's real, local rtl/
    directory (absolute path, derived from this file's own location --
    not a hardcoded /workspace/flow/src/ Docker-mount copy). Generated
    configs can land at any output_dir depth (see
    generate_variant_configs()), so a relative guess isn't safe here;
    the hand-written configs under flow/ use OpenLane's `dir::` prefix
    instead, see flow/config.tcl. See docs/architecture_spec.md and
    docs/baseline_reference.md for why the flow/src/ duplicate copy was
    removed 2026-09-07.
    """
    if not verilog_path:
        repo_root = Path(__file__).resolve().parent.parent
        verilog_path = str(repo_root / "rtl" / f"{cfg.rtl_module}.sv")
