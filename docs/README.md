# Documentation

| File | Purpose |
|---|---|
| [00_project_overview.md](00_project_overview.md) | Scope, stack, validation levels, research question |
| [01_status_and_roadmap.md](01_status_and_roadmap.md) | Experiment 0 checkpoint and next work |
| [architecture.md](architecture.md) | Adder RTL vs planned 8×4 PE array |
| [quantization.md](quantization.md) | Symmetric signed INT8 (and later INT4) |
| [verification.md](verification.md) | Python / RTL / physical checks |
| [physical_design.md](physical_design.md) | OpenLane stages, knobs, metrics |
| [experiment0_adder.md](experiment0_adder.md) | Adder GDSII evidence |
| [research_methodology.md](research_methodology.md) | Four-point study; hypotheses vs measurements |
| [github_and_lab_notebook.md](github_and_lab_notebook.md) | What to commit |

The technical manual is a tutorial, not the specification. Architecture and quantization decisions in these files override copy-paste RTL from the manual until audited.
